...
using namespace std;
// used to extract the images from the folder
...
Node< string> *currentImg; // image path of the current image which is to be displayed
const int size_of_arrows = 50; // height and width of arrows
int windowHeight = 0;
int windowWidth = 0;
 string imageFolder = "C:\\Users\\Dev\\Pictures"; // Path for the folder from where the images will be extracted
...
int index = 0;
 string value; // The final text that is displayed on the bottom right of screen
...
void LoadImagesFromFolder() {
	...
	if (!fs::exists(imageFolder) || !fs::is_directory(imageFolder)) {
		...
		imageFolder = fs::current_path().string() + "/images"; // Use "images" as the default subfolder
		 cout << "Specified folder not found. Using default folder path: " << imageFolder <<  endl;
	}
...
void LoadArrowImages() {
	...
	if (!arrow_left_default || !arrow_right_default || !arrow_left_hover || !arrow_right_hover) {
		 cerr << "Error loading arrow images: " << IMG_GetError() <<  endl;
		SDL_Quit();
		exit(-1);
	}
}
...
void RenderText(SDL_Surface* window_surface, const  string& text1, const  string& text2) {
	if (font == nullptr) {
...
{	
	...
	if (currentImg == nullptr) {
		 cerr << "Error: currentImg is nullptr." <<  endl;
		return false;
	}
	...
	if (surface != nullptr) {
		...
		value =  to_string(index + 1);
		value = value + " / " +  to_string(ImagePaths.length); // final text 
...
int main(int argc, char* argv[]) {
	...
	// Load a font
	 string FontsPath = fs::current_path().string() + "/Fonts/Inter/static/Inter-Medium.ttf";
	...
	value =  to_string(index + 1);
	value = value + " / " +  to_string(ImagePaths.length);
	RenderText(window_surface, currentImg->data.substr(imageFolder.length() + 1), value); // currentImg data is sliced to remove the folder and directory address and just show image name
